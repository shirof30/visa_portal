...
export default function VisaWizard() {
  ...
  const stepTitle = t(`wizardSteps.${stepId}`);
  const REVIEW_INDEX = STEP_IDS.findIndex((id) => id === "review");
  const progress = Math.round(((stepIndex + 1) / STEP_IDS.length) * 100);
  ...
  function jumpToStep(index: number) {
    ...
    if (index > stepIndex && !stepIsValid) {
      setAttempted(true);
      setError(t("errors.completeStep"));
      return;
    }
    setError(null); setAttempted(false); setEditFromReview(false);
  ...
  function handleEditStep(targetStepId: string) {
    const idx = STEP_IDS.findIndex((id) => id === targetStepId);
    if (idx < 0) return;
  ...
  async function handleSubmit() {
    if (!stepIsValid) {
      setAttempted(true);
      setError(t("errors.signBeforeSubmit"));
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        let detail = tCommon("tryAgain");
        try {
          const j = await res.json();
          if (j?.error) detail = j.error;
        } catch {}
        setError(t("errors.submitFailed", { detail }));
        setLoading(false);
        return;
      }
      ...
      setError(t("errors.submitFailed", { detail: e?.message ?? tCommon("networkError") }));
      setLoading(false);
    }
  }
      ...
      <div className="py-10 bg-security" style={{ minHeight: "100%" }}>
        ...
        <div className="mx-auto max-w-lg px-4">
          <div className="rounded-2xl border border-gray-200 bg-white shadow-[0_1px_3px_rgba(15,23,42,0.05),0_20px_48px_-20px_rgba(4,120,87,0.18)] p-8 text-center overflow-hidden relative">
            ...
            <p className="text-[10px] font-bold uppercase tracking-widest text-emerald-700 mb-1">{t("kjriLabel")}</p>
            <h1 className="text-xl font-extrabold tracking-tight text-gray-900 mb-2">{t("mailConfirmation.title")}</h1>
            <p className="text-sm text-gray-600 mb-6">
              {t("mailConfirmation.body")}
            </p>

            <div className="rounded-xl border border-gray-200 bg-gray-50 p-4 mb-6 text-left">
              <p className="text-[10px] font-bold uppercase tracking-wider text-gray-400 mb-1">{t("mailConfirmation.refLabel")}</p>
              <p className="font-mono text-lg font-bold text-gray-900">{mailConfirmation.ref}</p>
              <p className="text-xs text-gray-500 mt-1">{t("mailConfirmation.refHint")}</p>
            </div>
            ...
            >
              {t("mailConfirmation.checkStatus")}
            </Link>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="py-10 pb-32 bg-security" style={{ minHeight: "100%" }}>
          ...
          <div className="p-6 sm:p-8">
            ...
            {error && (
              <div className="mb-5 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
                <p className="font-semibold">{tCommon("attentionTitle")}</p>
            ...
            <form onSubmit={(e) => e.preventDefault()} className="space-y-4 text-sm" noValidate>
              ...
              <div className="mb-2 flex items-center justify-between">
                ...
                <div className="hidden md:flex items-center">
                  {STEP_IDS.map((id, i) => {
                    ...
                    const dotTitle = t("stepTitle", { current: i + 1, title: t(`wizardSteps.${id}`) });
                    return (
              ...
              {stepId === "otp" && (
                otpVerified ? (
                  <div className="rounded-2xl border border-green-200 bg-green-50 p-6 text-sm">
                    <p className="font-semibold text-green-700">{t("emailVerified.title")}</p>
                    <p className="mt-1 text-green-700/90">
                      {t("emailVerified.body", { email: form.email })}
                    </p>
                  </div>
                ) : (
                  <OtpStep
              ...
              <div className="flex items-center gap-3 pt-2">
                ...
                >
                  {editFromReview ? t("backToReview") : tCommon("back")}
                </button>
                ...
                {stepId !== "review" ? (
                  stepId !== "otp" && (
                    ...
                    >
                      {editFromReview ? tCommon("save") : tCommon("continue")}
                    </button>
                  )
                ) : (
                  <button
                    ...
                    className="w-full rounded-xl bg-emerald-600 px-4 py-3 text-sm font-semibold text-white shadow-sm transition-all duration-150 hover:bg-emerald-700 hover:-translate-y-px hover:shadow-lg hover:shadow-emerald-500/25 cursor-pointer disabled:opacity-60 disabled:hover:translate-y-0 disabled:cursor-not-allowed"
                  >
                    {loading ? tCommon("submitting") : t("submitApplication")}
                  </button>
                )}
              </div>
            </form>
        ...
        <div className="mx-auto max-w-3xl px-4 py-3">
          <div className="flex items-center gap-4">
            <div className="min-w-0 flex items-center gap-2.5">
              ...
              <div className="min-w-0">
                <p className="text-xs font-semibold text-gray-900">{t("progress.title")}</p>
                <p className="text-[11px] text-gray-500 truncate">
                  {t("progress.subtitle", { stepTitle, current: stepIndex + 1, total: STEP_IDS.length })}
                </p>
              </div>
            </div>
      ...
      <ProcessingOverlay
        open={loading && stepId === "review"}
        title={t("overlay.title")}
        subtitle={t("overlay.subtitle")}
        steps={t.raw("overlay.steps") as string[]}
      />
    </div>
  );
}
