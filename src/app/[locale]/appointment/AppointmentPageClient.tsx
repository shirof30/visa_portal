...
function SlotButton({
  ...
  let label: string;
  if (isCurrentBooking) label = t("slotLabels.yourSlot");
  else if (taken) label = t("slotLabels.full");
  else if (closedBySameDay) label = t("slotLabels.closed");
  else if (selected) label = t("slotLabels.selected");
  else label = t("slotLabels.available");
...
export default function AppointmentPageClient() {
  ...
  async function handleVerifyDob() {
    ...
    try {
      ...
      if (!res.ok) {
        setVerifyError((data as any).error ?? t("verification.failed"));
      } else {
  ...
  const handleBook = useCallback(async () => {
    setError(null);
    if (!selectedSlot) { setError(t("errors.selectSlot")); return; }
    if (isReschedule && !dob) { setError(t("errors.dobNotConfirmed")); return; }
    if (!isSameDayService && selectedDate < minBookingDateStr) { setError(t("errors.minBusinessDays")); return; }
    ...
    if (!res.ok) {
      const data = await res.json().catch(() => ({}));
      setError((data as any).error ?? t("errors.bookFailed"));
      setLoading(false);
      return;
    }
  ...
  }, [selectedSlot, isReschedule, isReferenceReschedule, isRegistrationReschedule, applicationRef, dob, isSameDayService, selectedDate, minBookingDateStr, registrationId, resolvedId, router, t]);

  const monthMatrix = useMemo(() => buildMonthMatrix(monthViewDate), [monthViewDate]);
  const monthLabel = monthViewDate.toLocaleString(dateLocale, { month: "long", year: "numeric" });
  ...
  if (missingId) return (
    <div className=" pt-24 flex items-center justify-center">
      <p className="text-sm text-gray-500">{t("missingId")}</p>
    </div>
  );
      ...
      <div className="w-full max-w-sm">
        <div className="rounded-2xl border border-gray-200 bg-white shadow-xl p-8">
          <div className="mb-6 text-center">
            ...
            <p className="text-xs font-bold uppercase tracking-widest text-red-600 mb-1">{t("verification.eyebrow")}</p>
            <h1 className="text-xl font-extrabold text-gray-900">{t("verification.title")}</h1>
      ...
      <div className="mx-auto max-w-5xl px-4">
          ...
          <div>
            <div className="inline-flex items-center gap-2 rounded-full border border-red-200 bg-red-50 px-3 py-1 mb-3">
              ...
              <span className="text-[10px] font-bold uppercase tracking-widest text-emerald-600">
                {isSameDayService ? t("badges.sameDay") : isReschedule ? t("badges.reschedule") : t("badges.book")}
              </span>
            </div>
            <h1 className="text-2xl font-bold text-gray-900 leading-tight">
              {isReschedule ? t("titleReschedule") : t("titleBook")}
            </h1>
            {submissionName && (
              <p className="text-sm text-gray-500 mt-1">
                {t("forApplicant")} <span className="font-semibold text-gray-700">{submissionName}</span>
              </p>
            )}
          </div>
          <button
          ...
          <div className="mb-4 flex items-center gap-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3">
            ...
            <p className="text-sm text-amber-800">
              ...
              <span className="font-bold">
                {new Date(initialBookedSlot.current.split("T")[0] + "T12:00:00").toLocaleDateString(dateLocale, { weekday: "short", day: "numeric", month: "short" })}
                {" · "}
        ...
        {isSameDayService && (
          <div className="mb-5 rounded-xl border border-amber-200 bg-amber-50/80 px-5 py-4">
            <p className="text-sm font-bold text-amber-900">{t("sameDayTitle")}</p>
            <p className="text-xs text-amber-700 mt-1">{t("sameDayHint")}</p>
          </div>
        )}
        ...
        <div className="grid gap-5 lg:grid-cols-2">
          ...
          <div className="rounded-2xl border border-gray-200/80 bg-white/75 backdrop-blur-xl shadow-xl shadow-black/5 overflow-hidden">
            <div className="border-b border-gray-100 px-5 py-4 bg-white/80">
              <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-0.5">{t("calendarTitle")}</p>
              <p className="text-sm font-semibold text-gray-800">{t("calendarSubtitle")}</p>
              {!isSameDayService && (
              ...
              {configLoading ? (
                ...
                <>
                  ...
                  <div className="mt-4 pt-4 border-t border-gray-100 flex flex-wrap gap-3 text-[10px] text-gray-400">
                    <span className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-emerald-600" />{t("legend.selected")}</span>
                    <span className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-amber-400" />{t("legend.holiday")}</span>
                    <span className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-gray-300" />{t("legend.full")}</span>
                    <span className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-amber-200 border border-amber-300" />{t("legend.yourSlot")}</span>
                  </div>
                </>
              )}
            </div>
          ...
          <div className="flex flex-col gap-4">
            <div className="rounded-2xl border border-gray-200/80 bg-white/75 backdrop-blur-xl shadow-xl shadow-black/5 overflow-hidden flex-1">
              <div className="border-b border-gray-100 px-5 py-4 bg-white/80 flex items-center justify-between">
                <div>
                  <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-0.5">{t("timeTitle")}</p>
                  <p className="text-sm font-semibold text-gray-800">
                    {new Date(selectedDate + "T12:00:00").toLocaleDateString(dateLocale, { weekday: "long", day: "numeric", month: "long", year: "numeric" })}
                  </p>
                </div>
                {selectedSlot && (
              ...
              <div className="px-5 py-5 space-y-6">
                {morningSlots.length > 0 && (
                  <div>
                    <div className="flex items-center justify-between mb-3">
                      <p className="text-xs font-bold uppercase tracking-wide text-gray-500">{t("morning")}</p>
                      <p className="text-[10px] text-gray-400">{isSameDayService ? t("morningHoursSameDay") : t("morningHoursDefault")}</p>
                    </div>
                ...
                {afternoonSlots.length > 0 && !isSameDayService && (
                  <div>
                    <div className="flex items-center justify-between mb-3">
                      <p className="text-xs font-bold uppercase tracking-wide text-gray-500">{t("afternoon")}</p>
                ...
                {morningSlots.length === 0 && afternoonSlots.length === 0 && (
                  <div className="flex flex-col items-center justify-center py-10 text-center">
                    ...
                    <p className="text-sm font-semibold text-gray-700">{t("noSlots")}</p>
                    <p className="text-xs text-gray-400 mt-1">{t("selectAnotherDate")}</p>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-gray-200/80 bg-white/75 backdrop-blur-xl shadow-xl shadow-black/5 px-5 py-4">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                  <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-1">{t("confirmTitle")}</p>
                  {selectedSlot ? (
                    <p className="text-sm font-bold text-gray-900">
                      {new Date(selectedDate + "T12:00:00").toLocaleDateString(dateLocale, { weekday: "short", day: "numeric", month: "short" })}
                      {" · "}
                      <span className="text-emerald-600">{selectedSlot.split("T")[1].slice(0, 5)}</span>
                    </p>
                  ) : (
                    <p className="text-sm text-gray-400">{t("noTimeSelected")}</p>
                  )}
                </div>
                <button
                  onClick={handleBook}
                ...
                >
                  ...
                  ) : selectedDate < minBookingDateStr ? t("selectAnotherDateButton") : t("confirmButton")}
                </button>
              </div>
            </div>
          </div>
