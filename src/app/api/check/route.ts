...
const STATUS_MAP: Partial<Record<ApplicationStatus, StatusVariant>> = {
  "Permohonan diterima": {
    ...
    nextStep: "Your application is being reviewed. Please attend your appointment as scheduled.",
  },
  "Permohonan disetujui": {
    label: "Approved", icon: "✓",
    accent: "#16a34a", accentLight: "#f0fdf4", accentText: "#15803d",
    nextStep: "Application approved. Please attend your appointment as scheduled.",
  },
  "Permohonan ditunda": {
    label: "On Hold", icon: "⏸",
    accent: "#d97706", accentLight: "#fffbeb", accentText: "#b45309",
    nextStep: "Action required. Please read the note below and contact us.",
  },
  "Permohonan ditolak": {
    label: "Rejected", icon: "✕",
    accent: "#059669", accentLight: "#ecfdf5", accentText: "#047857",
    nextStep: "Application could not be processed. Contact us for more information.",
  },
  "Permohonan sedang proses cetak": {
    label: "Processing", icon: "⬡",
    accent: "#7c3aed", accentLight: "#f5f3ff", accentText: "#6d28d9",
    nextStep: "Your visa is being processed. We will contact you shortly.",
  },
  "Paspor selesai diproses": {
    label: "Completed", icon: "★",
    accent: "#0891b2", accentLight: "#ecfeff", accentText: "#0e7490",
    nextStep: "Your visa is ready for collection at the consulate.",
  },
};
...
function linkify(text: string): React.ReactNode {
  ...
  return text.split(urlRegex).map((part, i) =>
    urlRegex.test(part)
      ? <a key={i} href={part} target="_blank" rel="noopener noreferrer" style={{ color: "#059669", textDecoration: "underline", wordBreak: "break-all" }}>{part}</a>
      : part
  );
}
...
export default function CheckPage() {
  ...
  async function handleCheck() {
    ...
    try {
      ...
      if (!res.ok) { setError(json.error || "Failed to check application."); return; }
      setResultKey(k => k + 1);
      setData(json as CheckResponse);
    } catch {
      setError("Failed to check application. Please try again.");
    } finally {
      setLoading(false);
    }
  }
      ...
      <div style={{ padding: "36px 16px 96px" }}>
        <div style={{ maxWidth: 440, margin: "0 auto" }}>
          ...
          <div style={{ ...card, padding: "20px 24px", marginBottom: 12, textAlign: "center" }}>
            <p style={{ margin: "0 0 6px", fontSize: 10, fontWeight: 700, letterSpacing: "0.22em", textTransform: "uppercase", color: "#059669" }}>
              Online Services · KJRI Vancouver
            </p>
          ...
          <div style={{ ...card, padding: 20, marginBottom: 12 }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <input
                ...
                onFocus={(e) => { e.target.style.borderColor = "#059669"; e.target.style.background = "white"; e.target.style.boxShadow = "0 0 0 3px rgba(5,150,105,0.08)"; }}
              ...
              <button
                ...
                style={{
                  ...
                  background: !canSubmit ? "#6ee7b7" : "#059669",
              ...
              >
                {loading
                  ? <><span style={{ width: 12, height: 12, border: "2px solid rgba(255,255,255,0.3)", borderTopColor: "white", borderRadius: "50%", display: "inline-block", animation: "spin 0.6s linear infinite" }} /> Check</>
                  : "Check →"}
              </button>
            </div>

            {error && (
              <div style={{ marginTop: 10, background: "#ecfdf5", border: "1px solid #fecaca", borderRadius: 10, padding: "10px 14px", display: "flex", gap: 8 }}>
                <span style={{ color: "#059669", flexShrink: 0 }}>⚠</span>
                <p style={{ margin: 0, fontSize: 12, color: "#047857", lineHeight: 1.5 }}>{error}</p>
              </div>
            )}
            ...
            <div style={{ marginTop: 14, paddingTop: 14, borderTop: "1px solid #f1f5f9", display: "flex", gap: 7 }}>
              ...
              <p style={{ margin: 0, fontSize: 12, color: "#94a3b8", lineHeight: 1.6 }}>
                ...
                <a href={`mailto:consular@indonesiavancouver.org?subject=${encodeURIComponent("Permintaan Reference Number Permohonan")}&body=${encodeURIComponent("Hello,\n\nI have forgotten my application reference number. Please help.\n\nFull name: [Your Name]\n\nThank you.")}`}
                ...
                <div style={{ background: "#0d2b5e", padding: "13px 20px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  ...
                  {data.appointmentSlot && daysUntil !== null && (
                    <span style={{
                      ...
                      background: daysUntil < 0 ? "rgba(255,255,255,0.1)" : daysUntil === 0 ? "#059669" : daysUntil <= 3 ? "#f59e0b" : "rgba(255,255,255,0.15)",
                      color: (daysUntil > 0 && daysUntil <= 3) ? "#1a1200" : "white",
                    }}>
                      {daysUntil < 0 ? "Past" : daysUntil === 0 ? "Today" : daysUntil === 1 ? "Tomorrow" : `${daysUntil} days away`}
                    </span>
                  )}
                </div>
                    ...
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          if (!reg) { setError("Registration number not available."); return; }
                          setPendingReg(reg);
                      ...
                      >
                        {locked ? "Cannot be changed" : "Reschedule →"}
                      </button>
                    </div>
                  </div>
                ) : (
                  ...
                  <div style={{ padding: "16px 20px", background: variant.accentLight }}>
                    ...
                    {(data.status === "Permohonan ditunda" || data.status === "Permohonan ditolak") && (
                      <a href="mailto:consular@indonesiavancouver.org"
                        style={{ display: "inline-flex", alignItems: "center", gap: 4, marginTop: 12, fontSize: 12, fontWeight: 700, color: "#059669", textDecoration: "none" }}>
                        Contact us →
                      </a>
                    )}
                  </div>
                  ...
                  <div style={{ padding: "16px 20px" }}>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 11 }}>
                      {([
                        ...
                        { icon: "📄", text: <span>Bring your original passport &amp; valid immigration documents <span style={{ color: "#64748b" }}>(PR Card, Study Permit, Work Permit, Visitor Records)</span></span> },
                        { icon: "📍", text: <span>1630 Alberni St, Vancouver, BC V6G 1A6</span> },
                        { icon: "👔", text: <span>Please dress in smart, collared attire — any colour <strong>except white</strong></span> },
      ...
      {showDobModal && (
        ...
        >
          <div className="modal-card" style={{
            ...
            <p style={{ margin: "0 0 4px", fontSize: 10, fontWeight: 700, letterSpacing: "0.2em", textTransform: "uppercase", color: "#059669" }}>
              KJRI Vancouver
            </p>
            ...
            <p style={{ margin: "0 0 24px", fontSize: 13, color: "#94a3b8", lineHeight: 1.6 }}>
              Enter your date of birth to access the appointment rescheduling page.
            </p>
            ...
            <div style={{ textAlign: "left", marginBottom: 20 }}>
              ...
              <input
                ...
                style={{
                  ...
                  border: `1.5px solid ${dobError ? "#6ee7b7" : "#e5e7eb"}`,
                ...
                onBlur={(e) => { e.target.style.borderColor = dobError ? "#6ee7b7" : "#e5e7eb"; e.target.style.boxShadow = "none"; }}
              />
              {dobError && (
                <p style={{ margin: "6px 0 0", fontSize: 12, color: "#059669", textAlign: "left" }}>{dobError}</p>
              )}
            </div>
            ...
            <button
              ...
              style={{
                ...
                background: !dobInput ? "#6ee7b7" : "#059669",
