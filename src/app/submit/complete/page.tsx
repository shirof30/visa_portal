      ...
      <div className="mx-auto max-w-2xl px-4">
        ...
        <div className="rounded-2xl border border-gray-200/80 bg-white/75 backdrop-blur-xl shadow-xl overflow-hidden mb-3">
          ...
          <div className="px-6 py-7 flex flex-col items-center text-center">
            ...
            <div className="inline-flex items-center gap-2 rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1 mb-3">
              ...
              <span className="text-[10px] font-bold uppercase tracking-widest text-emerald-700">Application Submitted</span>
            </div>
            <h1 className="text-2xl font-bold text-gray-900">Application Received</h1>
            <p className="text-sm text-gray-500 mt-2 max-w-sm">
              Your visa application has been submitted. Our consular team will review it and contact you if additional information is needed.
            </p>
            <div className="mt-3 flex items-center gap-2 rounded-full border border-emerald-200 bg-emerald-50 px-4 py-2">
              ...
              <p className="text-xs text-emerald-700">A confirmation email has been sent to your registered email address.</p>
            </div>
          </div>
        </div>
        ...
        {(fullName || reason) && (
          <div className="rounded-2xl border border-gray-200/80 bg-white/75 backdrop-blur-xl shadow-xl overflow-hidden mb-3">
            <div className="bg-[#0d2b5e] px-6 py-3">
              <span className="text-[10px] font-bold uppercase tracking-widest text-blue-200">Application Details</span>
            </div>
            <div className="px-6 py-4 flex flex-col sm:flex-row gap-4">
              {fullName && (
                <div className="flex-1">
                  <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-1">Applicant</p>
                  <p className="text-sm font-semibold text-gray-800">{fullName}</p>
                </div>
              )}
              {reason && (
                <div className="flex-1">
                  <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-1">Visa Type / Purpose</p>
        ...
        <div className="rounded-2xl border border-gray-200/80 bg-white/75 backdrop-blur-xl shadow-xl overflow-hidden mb-3">
          <div className="px-6 py-5">
            <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-3">Reference Number</p>
            ...
            <div className="flex items-start gap-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3">
              ...
              <p className="text-xs text-amber-800"><span className="font-bold">Important:</span> Save this reference number. You will need it to check your application status.</p>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-gray-200/80 bg-white/75 backdrop-blur-xl shadow-xl overflow-hidden mb-3">
          <div className="px-6 py-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {[
                { icon: "📋", title: "Review", desc: "Consular staff will review your application and documents." },
                { icon: "📞", title: "Contact", desc: "We may contact you for additional documents or an appointment." },
                { icon: "🏢", title: "Visit", desc: "You may need to visit 1630 Alberni St, Vancouver during office hours." },
              ].map(({ icon, title, desc }) => (
                <div key={title} className="rounded-xl border border-gray-100 bg-gray-50/80 p-4">
...
export default function CompletePage() {
  return (
    <Suspense fallback={<div className="py-20 flex items-center justify-center"><div className="h-8 w-8 rounded-full border-2 border-emerald-500 border-t-transparent animate-spin" /></div>}>
      <CompleteContent />
    </Suspense>
  );
}
